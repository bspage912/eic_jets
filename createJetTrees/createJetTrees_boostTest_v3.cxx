  ...
  for(int iEvent=0; iEvent<nevents; iEvent++)
    {
      ...
      //StEpSimuJetDef *akt03Pt_breit = event->newJetDef();
      ...
      // AKT Breit R = 0.3
      //akt03Pt_breit->setRadius(0.3);
      //akt03Pt_breit->setMinPt(0.250);
      //akt03Pt_breit->setAlgo(StEpSimuJetDef::AKT);
      //akt03Pt_breit->setFrame(StEpSimuJetDef::BREIT);

      // KT Breit
      kt10Pt_breit->setRadius(1.0);
      kt10Pt_breit->setMinPt(0.250);
      kt10Pt_breit->setAlgo(StEpSimuJetDef::KT);
      kt10Pt_breit->setFrame(StEpSimuJetDef::BREIT);

      // Cone Breit
      cone10Pt_breit->setRadius(1.0);
      cone10Pt_breit->setMinPt(0.250);
      ...
      //Double_t xP = inEvent->GetTrueX()*(1.0 + (inEvent->GetHardS())/(inEvent->GetTrueQ2()));
      //Double_t xP = (1.0)/(1.0 + (inEvent->GetHardS())/(inEvent->GetTrueQ2()));
      PseudoJet boost_vector = -(gamma + inEvent->GetTrueX()*proton);
      ...
      for(int j=0; j<inEvent->GetNTracks(); j++)
	{
      ...
      //JetDefinition jet_def_akt_03(antikt_algorithm,R_03);
      ...
      //ClusterSequence csPt_akt_03_breit(particlesPt_breit, jet_def_akt_03);
      ...
      //vector<PseudoJet> jetsPt_akt_03_breit = sorted_by_pt(csPt_akt_03_breit.inclusive_jets(ptmin));
      ...
      for(int i=0; i<jetsPt_akt_10_breit.size(); i++)
	{
      ...
      // AKT 07 Breit Pt Cut
      for(int i=0; i<jetsPt_akt_07_breit.size(); i++)
	{
      ...
      for(int i=0; i<jetsPt_akt_04_breit.size(); i++)
	{
      ...
      // AKT 03 Breit Pt Cut
      /*
      for(int i=0; i<jetsPt_akt_03_breit.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jetsPt_akt_03_breit[i].pt(),jetsPt_akt_03_breit[i].eta(),jetsPt_akt_03_breit[i].phi(),jetsPt_akt_03_breit[i].e());
	  akt03Pt_breit->addJet(jet);
	  jet->setJetDef(akt03Pt_breit);
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jetsPt_akt_03_breit[i].constituents();
      ...
      */
      ...
      // KT 10 Breit Pt Cut
      for(int i=0; i<jetsPt_kt_10_breit.size(); i++)
	{
      ...
      for(int i=0; i<jetsPt_cone_10_breit.size(); i++)
	{
