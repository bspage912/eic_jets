// Lab frame only with Radii = 1.0 0.8 0.4 for angularity studies
  ...
  for(int iEvent=0; iEvent<nevents; iEvent++)
    {
      ...
      StEpSimuJetDef *akt10 = event->newJetDef();
      StEpSimuJetDef *akt10Pt = event->newJetDef();
      StEpSimuJetDef *akt08 = event->newJetDef();
      StEpSimuJetDef *akt08Pt = event->newJetDef();
      StEpSimuJetDef *akt04 = event->newJetDef();
      ...
      // AKT BEAM
      akt10->setRadius(1.0);
      akt10->setMinPt(0.250);
      akt10->setAlgo(StEpSimuJetDef::AKT);
      akt10->setFrame(StEpSimuJetDef::BEAM);

      akt10Pt->setRadius(1.0);
      akt10Pt->setMinPt(0.500);
      akt10Pt->setAlgo(StEpSimuJetDef::AKT);
      akt10Pt->setFrame(StEpSimuJetDef::BEAM);

      akt08->setRadius(0.8);
      akt08->setMinPt(0.250);
      akt08->setAlgo(StEpSimuJetDef::AKT);
      akt08->setFrame(StEpSimuJetDef::BEAM);

      akt08Pt->setRadius(0.8);
      akt08Pt->setMinPt(0.500);
      akt08Pt->setAlgo(StEpSimuJetDef::AKT);
      akt08Pt->setFrame(StEpSimuJetDef::BEAM);

      akt04->setRadius(0.4);
      akt04->setMinPt(0.250);
      akt04->setAlgo(StEpSimuJetDef::AKT);
      akt04->setFrame(StEpSimuJetDef::BEAM);

      akt04Pt->setRadius(0.4);
      akt04Pt->setMinPt(0.500);
      ...
      for(unsigned int j=0; j<inEvent->GetNTracks(); j++)
	{
	  ...
	  if(j>10 && inParticle->GetStatus() == 1 && inParticle->GetParentIndex() != 3)
	      ...
	      if(TMath::Abs(inParticle->GetEta()) <= 4.0 && inParticle->GetPt() >= 0.250)
		{
		  ...
		  p.set_user_index(inParticle->GetIndex());
		  particles.push_back(p);
		  if(inParticle->GetPt() >= 0.500) particlesPt.push_back(p);
	  ...
	  */
      ...
      ClusterSequence cs_akt_10(particles, jet_def_akt_10);
      ...
      ClusterSequence cs_akt_08(particles, jet_def_akt_08);
      ClusterSequence csPt_akt_08(particlesPt, jet_def_akt_08);

      ClusterSequence cs_akt_04(particles, jet_def_akt_04);
      ...
      // Lab Frame Jets
      vector<PseudoJet> jets_akt_10 = sorted_by_pt(cs_akt_10.inclusive_jets(ptmin));
      vector<PseudoJet> jetsPt_akt_10 = sorted_by_pt(csPt_akt_10.inclusive_jets(ptmin));

      vector<PseudoJet> jets_akt_08 = sorted_by_pt(cs_akt_08.inclusive_jets(ptmin));
      vector<PseudoJet> jetsPt_akt_08 = sorted_by_pt(csPt_akt_08.inclusive_jets(ptmin));

      vector<PseudoJet> jets_akt_04 = sorted_by_pt(cs_akt_04.inclusive_jets(ptmin));
      ...
      // AKT 10 Pt > 250 MeV
      for(unsigned int i=0; i<jets_akt_10.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jets_akt_10[i].pt(),jets_akt_10[i].eta(),jets_akt_10[i].phi(),jets_akt_10[i].e());
	  akt10->addJet(jet);
	  jet->setJetDef(akt10);
	  ...
	  vector<PseudoJet> constituents = jets_akt_10[i].constituents();
      ...
      // Parton AKT 10 Pt > 500 MeV
      for(unsigned int i=0; i<jetsPt_akt_10.size(); i++)
	{
      ...
      // AKT 08 Pt > 250 MeV
      for(unsigned int i=0; i<jets_akt_08.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jets_akt_08[i].pt(),jets_akt_08[i].eta(),jets_akt_08[i].phi(),jets_akt_08[i].e());
	  akt08->addJet(jet);
	  jet->setJetDef(akt08);
	  //jet->setArea(jetsPt_akt_10[i].area());
	  //jet->setAreaError(jetsPt_akt_10[i].area_error());
	  ...
	  vector<PseudoJet> constituents = jets_akt_08[i].constituents();
      ...
      // Parton AKT 08 Pt > 500 MeV
      for(unsigned int i=0; i<jetsPt_akt_08.size(); i++)
	{
      ...
      // AKT 04 Pt Cut > 250 MeV
      for(unsigned int i=0; i<jets_akt_04.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jets_akt_04[i].pt(),jets_akt_04[i].eta(),jets_akt_04[i].phi(),jets_akt_04[i].e());
	  akt04->addJet(jet);
	  jet->setJetDef(akt04);
	  //jet->setArea(jetsPt_akt_10[i].area());
	  //jet->setAreaError(jetsPt_akt_10[i].area_error());
	  ...
	  vector<PseudoJet> constituents = jets_akt_04[i].constituents();
      ...
      // Parton AKT 04 Pt > 500 MeV
      for(unsigned int i=0; i<jetsPt_akt_04.size(); i++)
	{
