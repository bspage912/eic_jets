  ...
  for(int iEvent=0; iEvent<nevents; iEvent++)
    {
      ...
      StEpSimuJetDef *akt10_hbv = event->newJetDef();
      ...
      // AKT HBV
      akt10_hbv->setRadius(1.0);
      akt10_hbv->setMinPt(0.250);
      akt10_hbv->setAlgo(StEpSimuJetDef::AKT);
      akt10_hbv->setFrame(StEpSimuJetDef::GAMMA);

      akt10Pt_hbv->setRadius(1.0);
      akt10Pt_hbv->setMinPt(0.500);
      ...
      vector<PseudoJet> particles_hbv;
      ...
      for(unsigned int j=0; j<inEvent->GetNTracks(); j++)
	{
	  ...
	  if(j>10 && inParticle->GetStatus() == 1 && inParticle->GetParentIndex() != 3)
	      ...
	      if(inParticle->GetEta() <= 4.0 && inParticle->GetPt() >= 0.250)
		{
		  ...
		  fastjet::PseudoJet p_hbv(px_hbv,py_hbv,pz_hbv,E_hbv);
		  p_hbv.set_user_index(inParticle->GetIndex());
		  particles_hbv.push_back(p_hbv);
		  if(inParticle->GetPt() >= 0.500) particlesPt_hbv.push_back(p_hbv);
	  ...
	  //if(inParticle->GetParent() != NULL) 
	  //cout << inParticle->GetIndex() << " " << inParticle->GetStatus() << " " << inParticle->Id() << " " << inParticle->GetParentIndex() << " " << inParticle->GetParent()->Id() << endl;
      ...
      ClusterSequence cs_akt_10_hbv(particles_hbv, jet_def_akt_10);
      ...
      vector<PseudoJet> jets_akt_10_hbv = sorted_by_pt(cs_akt_10_hbv.inclusive_jets(ptmin));
      ...
      for(unsigned int i=0; i<jets_akt_10.size(); i++)
	{
	  ...
	  akt10->addJet(jet);
	  jet->setJetDef(akt10);
	  //jet->setArea(jets_akt_10[i].area());
	  //jet->setAreaError(jets_akt_10[i].area_error());
	  jet->setArea(-1.0);
      ...
      for(unsigned int i=0; i<jetsPt_akt_10.size(); i++)
	{
      ...
      // AKT 10 HB Pt Cut
      for(unsigned int i=0; i<jets_akt_10_hbv.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jets_akt_10_hbv[i].pt(),jets_akt_10_hbv[i].eta(),jets_akt_10_hbv[i].phi(),jets_akt_10_hbv[i].e());
	  akt10_hbv->addJet(jet);
	  jet->setJetDef(akt10_hbv);
	  //jet->setArea(jets_akt_10_hbv[i].area());
	  //jet->setAreaError(jets_akt_10_hbv[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jets_akt_10_hbv[i].constituents();
      ...
      for(unsigned int i=0; i<jetsPt_akt_10_hbv.size(); i++)
	{
      ...
      for(unsigned int i=0; i<jets_akt_10_breit.size(); i++)
	{
	  ...
	  //jet->setArea(jets_akt_10_breit[i].area());
	  //jet->setAreaError(jets_akt_10_breit[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);
