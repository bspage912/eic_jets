  ...
  for(int iEvent=0; iEvent<nevents; iEvent++)
    {
      ...
      StEpSimuJetDef *akt10Pt_parton = event->newJetDef();
      StEpSimuJetDef *akt08Pt = event->newJetDef();
      StEpSimuJetDef *akt08Pt_parton = event->newJetDef();
      StEpSimuJetDef *akt04Pt = event->newJetDef();
      StEpSimuJetDef *akt04Pt_parton = event->newJetDef();
      ...
      akt10Pt_parton->setRadius(1.0);
      akt10Pt_parton->setMinPt(0.001);
      akt10Pt_parton->setAlgo(StEpSimuJetDef::AKT);
      akt10Pt_parton->setFrame(StEpSimuJetDef::BEAM);

      akt08Pt->setRadius(0.8);
      akt08Pt->setMinPt(0.250);
      akt08Pt->setAlgo(StEpSimuJetDef::AKT);
      akt08Pt->setFrame(StEpSimuJetDef::BEAM);

      akt08Pt_parton->setRadius(0.8);
      akt08Pt_parton->setMinPt(0.001);
      akt08Pt_parton->setAlgo(StEpSimuJetDef::AKT);
      akt08Pt_parton->setFrame(StEpSimuJetDef::BEAM);

      akt04Pt->setRadius(0.4);
      akt04Pt->setMinPt(0.250);
      akt04Pt->setAlgo(StEpSimuJetDef::AKT);
      akt04Pt->setFrame(StEpSimuJetDef::BEAM);

      akt04Pt_parton->setRadius(0.4);
      akt04Pt_parton->setMinPt(0.001);
      akt04Pt_parton->setAlgo(StEpSimuJetDef::AKT);
      akt04Pt_parton->setFrame(StEpSimuJetDef::BEAM);
      ...
      vector<PseudoJet> partonsPt;
      vector<PseudoJet> partonsPt_hbv;
      vector<PseudoJet> partonsPt_breit;
      ...
      for(int j=0; j<inEvent->GetNTracks(); j++)
	{
	  ...
	  // Select Particles for Particle Jets
	  if(j>10 && inParticle->GetStatus() == 1 && inParticle->GetParentIndex() != 3)
	    {
	      if(TMath::Abs(inParticle->GetEta()) <= 4.5 && inParticle->GetPt() >= 0.250)
		{
	  ...
	  if(inParticle->GetStatus() != 1 && inParticle->GetStatus() != 21 && inParticle->GetParentIndex() != 3)
	    {

	      const Particle* parent = inParticle->GetParent();

	      if(TMath::Abs(inParticle->GetEta() <= 4.5) && inParticle->GetPt() >= 0.001 && parent->GetStatus() == 21)
		{
		  fastjet::PseudoJet pPt(px,py,pz,E);
		  pPt.set_user_index(inParticle->GetIndex());
		  partonsPt.push_back(pPt);
		  
		  fastjet::PseudoJet pPt_hbv(px_hbv,py_hbv,pz_hbv,E_hbv);
		  pPt_hbv.set_user_index(inParticle->GetIndex());
		  partonsPt_hbv.push_back(pPt_hbv);

		  boosted_particlePt.set_user_index(inParticle->GetIndex());
		  partonsPt_breit.push_back(boosted_particlePt);
		}
	    }
      ...
      JetDefinition jet_def_akt_08(antikt_algorithm,R_08);
      ...
      ClusterSequence csPt_parton_akt_10(partonsPt, jet_def_akt_10);

      ClusterSequence csPt_akt_08(particlesPt, jet_def_akt_08);
      ClusterSequence csPt_parton_akt_08(partonsPt, jet_def_akt_08);

      ClusterSequence csPt_akt_04(particlesPt, jet_def_akt_04);
      ClusterSequence csPt_parton_akt_04(partonsPt, jet_def_akt_04);
      ...
      vector<PseudoJet> jetsPt_parton_akt_10 = sorted_by_pt(csPt_parton_akt_10.inclusive_jets(ptmin));

      vector<PseudoJet> jetsPt_akt_08 = sorted_by_pt(csPt_akt_08.inclusive_jets(ptmin));
      vector<PseudoJet> jetsPt_parton_akt_08 = sorted_by_pt(csPt_parton_akt_08.inclusive_jets(ptmin));

      vector<PseudoJet> jetsPt_akt_04 = sorted_by_pt(csPt_akt_04.inclusive_jets(ptmin));
      vector<PseudoJet> jetsPt_parton_akt_04 = sorted_by_pt(csPt_parton_akt_04.inclusive_jets(ptmin));
      ...
      //cout << jetsPt_akt_10.size() << " " << jetsPt_parton_akt_10.size() << " " << jetsPt_akt_08.size() << " " << jetsPt_parton_akt_08.size() << endl;
      ...
      for(int i=0; i<jetsPt_akt_10.size(); i++)
	{
      ...
      // Parton AKT 10 Pt Cut
      for(int i=0; i<jetsPt_parton_akt_10.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jetsPt_parton_akt_10[i].pt(),jetsPt_parton_akt_10[i].eta(),jetsPt_parton_akt_10[i].phi(),jetsPt_parton_akt_10[i].e());
	  akt10Pt_parton->addJet(jet);
	  jet->setJetDef(akt10Pt_parton);
	  //jet->setArea(jetsPt_akt_10[i].area());
	  //jet->setAreaError(jetsPt_akt_10[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jetsPt_parton_akt_10[i].constituents();
      ...
      // AKT 08 Pt Cut
      for(int i=0; i<jetsPt_akt_08.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jetsPt_akt_08[i].pt(),jetsPt_akt_08[i].eta(),jetsPt_akt_08[i].phi(),jetsPt_akt_08[i].e());
	  akt08Pt->addJet(jet);
	  jet->setJetDef(akt08Pt);
	  //jet->setArea(jetsPt_akt_10[i].area());
	  //jet->setAreaError(jetsPt_akt_10[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);
	  ...
	  vector<PseudoJet> constituents = jetsPt_akt_08[i].constituents();
      ...
      // Parton AKT 08 Pt Cut
      for(int i=0; i<jetsPt_parton_akt_08.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jetsPt_parton_akt_08[i].pt(),jetsPt_parton_akt_08[i].eta(),jetsPt_parton_akt_08[i].phi(),jetsPt_parton_akt_08[i].e());
	  akt08Pt_parton->addJet(jet);
	  jet->setJetDef(akt08Pt_parton);
	  //jet->setArea(jetsPt_akt_10[i].area());
	  //jet->setAreaError(jetsPt_akt_10[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  //cout << "IN 08 PARTON" << endl;

	  vector<PseudoJet> constituents = jetsPt_parton_akt_08[i].constituents();
      ...
      // AKT 04 Pt Cut
      for(int i=0; i<jetsPt_akt_04.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jetsPt_akt_04[i].pt(),jetsPt_akt_04[i].eta(),jetsPt_akt_04[i].phi(),jetsPt_akt_04[i].e());
	  akt04Pt->addJet(jet);
	  jet->setJetDef(akt04Pt);
	  //jet->setArea(jetsPt_akt_10[i].area());
	  //jet->setAreaError(jetsPt_akt_10[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jetsPt_akt_04[i].constituents();
      ...
      // Parton AKT 04 Pt Cut
      for(int i=0; i<jetsPt_parton_akt_04.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jetsPt_parton_akt_04[i].pt(),jetsPt_parton_akt_04[i].eta(),jetsPt_parton_akt_04[i].phi(),jetsPt_parton_akt_04[i].e());
	  akt04Pt_parton->addJet(jet);
	  jet->setJetDef(akt04Pt_parton);
	  //jet->setArea(jetsPt_akt_10[i].area());
	  //jet->setAreaError(jetsPt_akt_10[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jetsPt_parton_akt_04[i].constituents();
