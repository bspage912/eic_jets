...
int main(int argc, char* argv[]) {
  ...
  for(int iEvent=0; iEvent<nevents; iEvent++)
    {
      ...
      StEpSimuJetDef *eekt10Pt = event->newJetDef();
      StEpSimuJetDef *eekt10Pt_breit = event->newJetDef();
      //StEpSimuJetDef *akt10Pt = event->newJetDef();
      ...
      // EEKT Beam
      eekt10Pt->setRadius(0.0);
      eekt10Pt->setMinPt(0.250);
      eekt10Pt->setAlgo(StEpSimuJetDef::AKT);
      eekt10Pt->setFrame(StEpSimuJetDef::BEAM);

      // EEKT Breit
      eekt10Pt->setRadius(0.0);
      eekt10Pt->setMinPt(0.250);
      eekt10Pt->setAlgo(StEpSimuJetDef::AKT);
      eekt10Pt->setFrame(StEpSimuJetDef::BREIT);
      ...
      for(int j=0; j<inEvent->GetNTracks(); j++)
	{
	  ...
	  myParticle->setFourMom_breit(boosted_particlePt.px(),boosted_particlePt.py(),boosted_particlePt.pz(),boosted_particlePt.e());
      ...
      /*
      // Check 4 1 Particle
      if(particlesPt_breit.size() < 2)
	{
	  cout << "Event = " << iEvent << endl;
	  cout << "ONLY ONE PARTICLE" << endl;
	  cout << "PROCESS = " << inEvent->GetProcess() << endl;

	  for(int j=0; j<inEvent->GetNTracks(); j++)
	    {
	      const Particle* inParticle = inEvent->GetTrack(j);

	      cout << "Index Status PDG Parent Pt Eta: " << inParticle->GetIndex() << " " << inParticle->GetStatus() << " " << inParticle->GetPdgCode() << " " << inParticle->GetParentIndex() << " " << inParticle->GetPt() << " " << inParticle->GetEta() << endl;
	    }
	  cout << endl;
	}
      */

      // Don't Find Jets on One Particle Events
      if(particlesPt.size() > 1)
	{
	  onePartNum++;
	  //continue;
	  ...
	  JetDefinition jet_def_eekt(ee_kt_algorithm);
	  ...
	  ClusterSequence csPt_eekt(particlesPt, jet_def_eekt);
	  ...
	  ClusterSequence csPt_eekt_breit(particlesPt_breit, jet_def_eekt);
	  ...
	  vector<PseudoJet> jetsPt_eekt = sorted_by_pt(csPt_eekt.exclusive_jets(2));
	  ...
	  vector<PseudoJet> jetsPt_eekt_breit = sorted_by_pt(csPt_eekt_breit.exclusive_jets(2));
	  ...
	  */
	  ...
	  // EEKT 10 Pt Cut
	  for(int i=0; i<jetsPt_eekt.size(); i++)
	    {
	      StEpSimuJet *jet = event->newJet(jetsPt_eekt[i].pt(),jetsPt_eekt[i].eta(),jetsPt_eekt[i].phi(),jetsPt_eekt[i].e());
	      eekt10Pt->addJet(jet);
	      jet->setJetDef(eekt10Pt);
	      //jet->setArea(jetsPt_akt_10[i].area());
	      //jet->setAreaError(jetsPt_akt_10[i].area_error());
	      jet->setArea(-1.0);
	      jet->setAreaError(-1.0);
	      
	      vector<PseudoJet> constituents = jetsPt_eekt[i].constituents();
	  ...
	  // EEKT 10 Breit Pt Cut
	  for(int i=0; i<jetsPt_eekt_breit.size(); i++)
	    {
	      StEpSimuJet *jet = event->newJet(jetsPt_eekt_breit[i].pt(),jetsPt_eekt_breit[i].eta(),jetsPt_eekt_breit[i].phi(),jetsPt_eekt_breit[i].e());
	      eekt10Pt_breit->addJet(jet);
	      jet->setJetDef(eekt10Pt_breit);
	      //jet->setArea(jetsPt_akt_10_breit[i].area());
	      //jet->setAreaError(jetsPt_akt_10_breit[i].area_error());
	      jet->setArea(-1.0);
	      jet->setAreaError(-1.0);
	      
	      vector<PseudoJet> constituents = jetsPt_eekt_breit[i].constituents();
  ...
  cout << "Number of Multi-Particle Events = " << onePartNum << endl;
