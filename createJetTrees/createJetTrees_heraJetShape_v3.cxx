  ...
  for(int iEvent=0; iEvent<nevents; iEvent++)
    {
      ...
      StEpSimuJetDef *sisCone10 = event->newJetDef();
      StEpSimuJetDef *cdfCone10 = event->newJetDef();
      ...
      // SIScone Lab
      sisCone10->setRadius(1.0);
      sisCone10->setMinPt(0.150);
      sisCone10->setAlgo(StEpSimuJetDef::AKT);
      sisCone10->setFrame(StEpSimuJetDef::BEAM);

      cdfCone10->setRadius(1.0);
      cdfCone10->setMinPt(0.150);
      cdfCone10->setAlgo(StEpSimuJetDef::KT);
      cdfCone10->setFrame(StEpSimuJetDef::BEAM);

      akt10->setRadius(1.0);
      akt10->setMinPt(0.150);
      ...
      for(unsigned int j=0; j<inEvent->GetNTracks(); j++)
	{
	  ...
	  if(j>10 && inParticle->GetStatus() == 1 && inParticle->GetParentIndex() != 3)
	    {
	      if(TMath::Abs(inParticle->GetEta()) <= 3.0 && inParticle->GetPt() >= 0.150)
		{
		  fastjet::PseudoJet pPt(px,py,pz,E);
		  pPt.set_user_index(inParticle->GetIndex());
		  particles.push_back(pPt);
      ...
      double seed_thresh = 0.300;

      JetDefinition::Plugin *sisPlug = new SISConePlugin(R_10,overlap_threshold);
      JetDefinition::Plugin *cdfPlug = new CDFMidPointPlugin(R_10,overlap_threshold,seed_thresh,1.0);

      JetDefinition jet_def_sis_10(sisPlug);
      JetDefinition jet_def_cdf_10(cdfPlug);
      ...
      ClusterSequence cs_sis_10(particles, jet_def_sis_10);
      ClusterSequence cs_cdf_10(particles, jet_def_cdf_10);
      ...
      // Breit Frame Jets
      vector<PseudoJet> jets_sis_10 = sorted_by_pt(cs_sis_10.inclusive_jets(ptmin));
      vector<PseudoJet> jets_cdf_10 = sorted_by_pt(cs_cdf_10.inclusive_jets(ptmin));
      ...
      // AKT 10 Breit Pt Cut
      for(unsigned int i=0; i<jets_sis_10.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jets_sis_10[i].pt(),jets_sis_10[i].eta(),jets_sis_10[i].phi(),jets_sis_10[i].e());
	  sisCone10->addJet(jet);
	  jet->setJetDef(sisCone10);
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jets_sis_10[i].constituents();
      ...
      for(unsigned int i=0; i<jets_cdf_10.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jets_cdf_10[i].pt(),jets_cdf_10[i].eta(),jets_cdf_10[i].phi(),jets_cdf_10[i].e());
	  cdfCone10->addJet(jet);
	  jet->setJetDef(cdfCone10);
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jets_cdf_10[i].constituents();
