  ...
  for(int iEvent=0; iEvent<nevents; iEvent++)
    {
      ...
      StEpSimuJetDef *akt10Pt_breitT = event->newJetDef();
      ...
      // AKT Breit True X
      akt10Pt_breitT->setRadius(1.0);
      akt10Pt_breitT->setMinPt(0.250);
      akt10Pt_breitT->setAlgo(StEpSimuJetDef::AKT);
      akt10Pt_breitT->setFrame(StEpSimuJetDef::BREIT);
      ...
      vector<PseudoJet> particlesPt_breitT;
      ...
      double bxL = boost_vectorE.px()/boost_vectorE.E(); // Check for valid boost
      double byL = boost_vectorE.py()/boost_vectorE.E(); // Check Electron Method Only
      double bzL = boost_vectorE.pz()/boost_vectorE.E();
      ...
      if(bxL*bxL + byL*byL + bzL*bzL >= 1.0) 
	{
      ...
      PseudoJet boosted_protonT = boost(proton,boost_vectorT);
      Double_t phi_pT = boosted_protonT.phi();
      Double_t theta_pT = TMath::ATan2(boosted_protonT.perp(),boosted_protonT.pz());
      ...
      for(int j=0; j<inEventS->GetNTracks(); j++)
	{
	  ...
	  if(inParticleS == NULL) // Particle was not smeared, use original
	    {	  
	      ...
	      if(j>10 && inParticle->GetStatus() == 1 && TMath::Abs(inParticle->GetEta()) < 4.5)
		{
		  //cout << "ID PseudoRapidity = " << inParticle->GetPdgCode() << " " << inParticle->GetEta() << endl;
		}
	    }
	  ...
	  if(inParticleS != NULL) // Particle was smeared
	      ...
	      if(localCode == 11 || localCode == 22 || localCode == 2112 || localCode == 130)
		{
	      ...
	      if(localCode > 110 && localCode != 2112 && localCode != 130 && TMath::Abs(inParticle->GetEta()) > 3.5 && TMath::Abs(inParticle->GetEta()) <= 4.5)
		{
		  Double_t factor2 = (tmpE*tmpE)/(tmpX*tmpX + tmpY*tmpY + tmpZ*tmpZ);
	  ...
	  fastjet::PseudoJet p_labT(px,py,pz,E);
	  fastjet::PseudoJet boosted_particleT = boost(p_labT,boost_vectorT);
	  boosted_particleT = rotateZ(boosted_particleT, -phi_pT);
	  boosted_particleT = rotateY(boosted_particleT, -theta_pT);
	      ...
	      if(inParticleS == NULL) // Not Smeared
		{
		  if(inParticle->GetEta() <= 4.5 && inParticle->GetPt() >= 0.250)
		      ...
		      if(neutralHadronAccept == 1 && trackAccept == 1)
			{
			  ...
			  boosted_particleT.set_user_index(inParticle->GetIndex());
			  particlesPt_breitT.push_back(boosted_particleT);
	  ...
	  myParticle->setFourMom_hb(0.,0.,0.,0.);
	  myParticle->setFourMom_breit(boosted_particleE.px(),boosted_particleE.py(),boosted_particleE.pz(),boosted_particleE.e());
      ...
      ClusterSequence csPt_akt_10_breitT(particlesPt_breitT, jet_def_akt_10);
      //ClusterSequenceArea csPt_akt_10_breit(particlesPt_breit, jet_def_akt_10, area_def);
      ...
      vector<PseudoJet> jetsPt_akt_10_breitT = sorted_by_pt(csPt_akt_10_breitT.inclusive_jets(ptmin));
      ...
      for(int i=0; i<jetsPt_akt_10_breitDA.size(); i++)
	{
      ...
      for(int i=0; i<jetsPt_akt_10_breitT.size(); i++)
	{
	  StEpSimuJet *jet = event->newJet(jetsPt_akt_10_breitT[i].pt(),jetsPt_akt_10_breitT[i].eta(),jetsPt_akt_10_breitT[i].phi(),jetsPt_akt_10_breitT[i].e());
	  akt10Pt_breitT->addJet(jet);
	  jet->setJetDef(akt10Pt_breitT);
	  //jet->setArea(jetsPt_akt_10_breit[i].area());
	  //jet->setAreaError(jetsPt_akt_10_breit[i].area_error());
	  jet->setArea(-1.0);
	  jet->setAreaError(-1.0);

	  vector<PseudoJet> constituents = jetsPt_akt_10_breitT[i].constituents();
